...
async def handler():
    while True:
        ...
        try:
            await process_file(filename, filepath, author)
...
def process_file(filename: str, filepath: str, author: str):
    row_count, incorrect_rows = validate_rows(filepath)
    ...
    try:
        ...
        file_id = add_file_info(filename, author, db)

        # добавлние корректных строк в таблицу значений
        add_values(filepath, file_id, incorrect_rows, db)

        # вычисление показателей и запись в таблицу результатов
        add_result(file_id, db)
        db.commit()
    except Exception:
        db.rollback()
        raise
...
def validate_rows(filepath: str):
    row_count = 0
    incorrect_rows = []
...
def add_file_info(filename: str, author: str, db: Session):
...
def add_result(file_id: int, db: Session):
